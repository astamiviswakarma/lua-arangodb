...
local function new_wal_api(schemes, host, port, basePath, timeout)
	local schemes_map = {}
...
function wal_api:admin_wal_flush_put(wait_for_sync, wait_for_collector)
...
function wal_api:admin_wal_properties_get()
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function wal_api:admin_wal_properties_put()
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function wal_api:admin_wal_transactions_get()
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
